...
const BuyCard = ({ cards, setCards }) => {
    ...
    return (
        <div className='w-11/12 sm:w-10/12 mx-auto shadow-lg rounded-lg border border-amber-100 p-5 sm:p-10 mb-10 space-y-3'>
            ...
            {
                cards.length === 0 ? <div className=' p-10 text-center'><p className='text-2xl font-semibold opacity-70'>Card is empty</p></div> :
                    <> 
                        {
                            cards.map((item, index) =>
                                <div key={index} className='flex justify-between items-center shadow-lg rounded-lg border border-amber-100 p-2 sm:p-6'>
                                    ...
                                    <div
                                    onClick={()=>hendelDelete(item)}
                                    className='btn bg-transparent border-none text-xl sm:text-2xl text-red-700 '>
                                        <MdDelete />
                                    </div>
                                </div>
                            )
                        ...
                        <button
                            onClick={hendelPyament}
                            className='btn w-full text-white rounded-full bg-linear-to-r from-[#4F39F6] to-[#9514FA]'>Proceed to Checkout</button>
